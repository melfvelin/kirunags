...
namespace utils
{
	...
	*/
	...
	/*	GetMsgType - retrieves the message type from any Expedite frame and returns it as int
	*	inputs: uint8_t pointer to frame
	*	outputs: -1 on failure, > -1 on success
	*	function calls: none
	*	author: Martin Elfvelin
	*/
	int GetMsgType(uint8_t *pnPacket)
	{
		GEN_HEADER m_sGenHeader;

		memcpy(&m_sGenHeader, pnPacket, sizeof(GEN_HEADER));

		if(m_sGenHeader.nMsgType >= 0 && m_sGenHeader.nMsgType <= 11)
		{
			return (m_sGenHeader.nMsgType & 0xFF);
		}
		else
		{
			return -1;
	...
	namespace parse
	{
		...
		/*	ParseMsgType - returns the message type in plain text
		*	inputs: uint32_t reference to message type
		*	outputs: returns plain text string (message type)
		*	function calls: none
		*	author: Martin Elfvelin
		*/			
		...
		/*	ParseScrambler - returns the scrambler in plain text
		*	inputs: uint32_t reference to scrambler
		*	outputs: returns plain text string (scramber)
		*	function calls: none
		*	author: Martin Elfvelin
		*/
		...
		/*	ParseFEC - returns the forward error correction in plain text
		*	inputs: uint32_t reference to FEC
		*	outputs: returns plain text string (FEC)
		*	function calls: none
		*	author: Martin Elfvelin
		*/
		...
		/*	ParseFrameFormat - returns the frame format in plain text
		*	inputs: uint32_t reference to frame format
		*	outputs: returns plain text string (frame format)
		*	function calls: none
		*	author: Martin Elfvelin
		*/
