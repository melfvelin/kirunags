...
// parseLines gets parameters from a TLE and passes those parameters to rec object
void TLE::parseLines(char *line1, char *line2)
{
    ...
    TLE::setValsToRec(this, this->rec);
}
...
/*  parseEpoch - takes an epoch string (year:doy:fractionalday)
*   from TLE and extracts time data to elsetrec struct rec
*
*/
long parseEpoch(elsetrec *rec, char *str)
{
    ...
    SGP4Funcs::jday_SGP4(year, mon, day, hr, mn, sec, rec->jdsatepoch, rec->jdsatepochF);

    double diff = rec->jdsatepoch - 2440587.5;
    double diff2 = 86400000.0*rec->jdsatepochF;
    diff*=86400000.0;

    // returns milisec since 1950 in a long
...
// getRV outputs position and velocity vectors by calling sgp4 function
void TLE::getRV(double minutesAfterEpoch, double r[3], double v[3])
{
    this->rec.error = 0;
    SGP4Funcs::sgp4(this->rec, minutesAfterEpoch, r, v);
...
void TLE::setValsToRec(TLE *tle, elsetrec& rec)
{
...
// try replacing -> with .
    ...
    strncpy(rec.satnum,tle->objectID,5);
    rec.bstar = tle->bstar;
    rec.inclo = deg2Rad(tle->incDeg);
    rec.nodeo = deg2Rad(tle->raanDeg);
    rec.argpo = deg2Rad(tle->argpDeg);
    rec.mo = deg2Rad(tle->maDeg);
    rec.ecco = tle->ecc;
    rec.no_kozai = tle->n/xpdotp;
    rec.ndot = tle->ndot / (xpdotp*1440.0d);
    rec.nddot = tle->nddot / (xpdotp*1440.0d*1440.0d);
        
    SGP4Funcs::sgp4init(wgs72, 'a', rec.satnum, (rec.jdsatepoch + rec.jdsatepochF - 2433281.5)
    , rec.bstar,
        rec.ndot, rec.nddot, rec.ecco, rec.argpo, rec.inclo, rec.mo,
        rec.no_kozai, rec.nodeo, rec);
}
