...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    // setting rf frequency by creating tune request
    uhd::tune_request_t tune_request;
    // assigning freq to tune request
    tune_request = uhd::tune_request_t(freq, lo_offset);
    if (vm.count("int-n"))
        tune_request.args = uhd::device_addr_t("mode_n=integer");
    // setting frequency with tune request
    usrp->set_tx_freq(tune_request);
    // printing out TX frequency, should only be done in the beginning
    ...
    do {
        ...
        if (repeat and delay > 0.0) {
            ...
            // this could be used to adjust the tx frequency
            freq = freq + 100000;
            tune_request = uhd::tune_request_t(freq, lo_offset);
            usrp->set_tx_freq(tune_request);
